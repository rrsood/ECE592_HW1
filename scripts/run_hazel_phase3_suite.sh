...
usage() {
  echo "usage: $0 --constraint NAME --generation NAME [--freeze-directory DIR --freeze-commit HASH | --unfrozen-run] --result-root DIR" >&2
  exit 2
}
...
run_id="${constraint}_${SLURM_JOB_ID}"
...
python3 scripts/generate_conflict_plan.py \
  --conflict-strides 4096,8192,16384,32768,65536,131072,262144,524288,1048576 \

#!/usr/bin/env python3
"""Validate and summarize every point in a completed spatial-locality sweep."""
...
def validate_manifest(plan_rows, manifest_rows):
    ...
    for plan_row, manifest_row in zip(plan_rows, manifest_rows):
        ...
        if manifest_row["status"] != "complete" or manifest_row["return_code"] != 0:
            raise ValueError(
                "spatial sweep is not complete at global index {}".format(
...
def validate_raw_point(metadata, plan_row, raw_filename, build_command,
    ...
    require_raw_integer(metadata, "region_count", plan_row["region_count"])
    require_raw_integer(
        metadata, "region_spacing_bytes", plan_row["region_spacing_bytes"])
    require_raw_integer(
        metadata, "probe_offset_bytes", plan_row["probe_offset_bytes"])
    require_raw_integer(metadata, "random_seed", plan_row["point_seed"])
    require_raw_integer(
        metadata, "dependent_nodes_per_cycle", 2 * plan_row["region_count"])

    minimum_warmup_batches = (
        2 * plan_row["region_count"] + accesses_per_sample - 1) // accesses_per_sample
    ...
    if raw_warmup_batches < minimum_warmup_batches:
        raise ValueError(
            "raw warm-up does not cover one complete spatial cycle")

    for key in COMMON_RAW_KEYS:
        if key not in metadata:
            raise ValueError("raw file is missing common metadata {}".format(key))

    if metadata["traversal"] != "randomized-spatial-dependent-cycle":
...
def process_points(run_directory, plan_rows, manifest_rows, build_command):
    ...
    for plan_row, manifest_row in zip(plan_rows, manifest_rows):
        ...
        row = {
            ...
            "region_count": plan_row["region_count"],
            "region_spacing_bytes": plan_row["region_spacing_bytes"],
            "probe_offset_bytes": plan_row["probe_offset_bytes"],
...
def write_processed(path, run_directory, plan_path, manifest_path,
    ...
    try:
        with open(path, "x", encoding="utf-8", newline="\n") as stream:
            created = True
            stream.write("ece592_spatial_processed_format_version=1\n")
            stream.write("source_run_directory={}\n".format(run_directory))
            stream.write("source_spatial_plan_sha256={}\n".format(
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Validate and summarize a completed spatial-run directory.")
...
def main():
    ...
    plan_path = os.path.join(arguments.run_directory, "spatial_plan.tsv")
    ...
    try:
        ...
        plan_metadata, numeric_plan_metadata, plan_rows = (
            run_spatial_plan.read_plan(plan_path))
        del numeric_plan_metadata
        build_command = run_spatial_plan.read_build_command(build_command_path)

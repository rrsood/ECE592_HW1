...
def main():
    ...
    for row in selected:
        ...
        path = os.path.join(args.output_directory, "hazel_{}.sbatch".format(constraint))
        with open(path, "x", encoding="utf-8", newline="\n") as stream:
            stream.write("#!/bin/bash\n")
            stream.write("#SBATCH --job-name=ece592_{}\n".format(constraint))
            ...
            stream.write("srun --cpu-bind=verbose,cores --cpus-per-task=1 bash scripts/run_hazel_phase3_suite.sh \\\n")
    ...
    print("generated_job_count={}".format(len(selected)))

#!/usr/bin/env bash
#
# Build the Section 8.2.7 inclusion/exclusion benchmark.
#
# This mirrors scripts/build_phase1.sh: same -O0 baseline, same warning set,
# same recorded-build-command sidecar file, same architecture-specific timer
# selection.  The only additional flag is -pthread, which the helper-thread
# pressure source requires.
#
# usage: scripts/build_inclusion_policy.sh <absolute-output-executable>
...
sources=(
    main_code/common/inclusion_policy_bench.c
    main_code/common/cross_level.c

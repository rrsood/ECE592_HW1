#!/usr/bin/env python3
"""Create a specification-blind Phase-I spatial-locality curve."""
...
def read_processed(path):
    ...
    with open(path, "r", encoding="utf-8", newline="") as stream:
        ...
        for record in reader:
            ...
            if (row["trial"] < 0 or row["plot_index"] < 0 or
                    row["offset"] <= 0 or row["sample_count"] < 1_000_000 or
                    not all(math.isfinite(row[key])
                            for key in ("median", "q1", "q3")) or
                    not row["q1"] <= row["median"] <= row["q3"]):
                raise ValueError("invalid processed spatial row")
            rows.append(row)
    ...
    required_metadata = {
        "ece592_spatial_processed_format_version",
        "source_raw_hostname",
        "source_raw_isa",
    ...
    if metadata["ece592_spatial_processed_format_version"] != "1":
        raise ValueError("unsupported processed format version")
    if metadata["timer_overhead_subtracted"] != "false":
        raise ValueError("spatial curve requires unadjusted timer values")
    if metadata["source_raw_traversal"] != \
            "randomized-spatial-dependent-cycle":
        raise ValueError("spatial curve must use randomized spatial traversal")
...
def aggregate_rows(trial_count, rows):
    ...
    for offset in sorted(by_offset):
        point_rows = sorted(by_offset[offset], key=lambda row: row["trial"])
        if len(point_rows) < 1:
            raise ValueError("probe offset has no measurements")
        if any(row["trial"] < 0 or row["trial"] >= trial_count
               for row in point_rows):
            raise ValueError("probe offset contains invalid trial number")
...
def tick_values(minimum, maximum):
    ...
    return [
        ...
        if minimum <= (1 << power) <= maximum
    ]


def write_plot_data(path, aggregated, maximum_repeat_count):
    with open(path, "w", encoding="ascii", newline="\n") as stream:
        headings = ["probe_offset_bytes", "aggregate_median", "iqr_low",
                    "iqr_high"]
        ...
        for point in aggregated:
            values = [
                point["offset"], point["aggregate_median"],
...
def write_gnuplot_script(path, data_path, temporary_pdf, metadata,
    ...
    ticks = tick_values(aggregated[0]["offset"], aggregated[-1]["offset"])
    tick_text = ", ".join(
        "{} {}".format(gnuplot_quote(str(value)), value)
        for value in ticks)
    title = "{} Phase I randomized spatial-locality sweep".format(
    ...
    if style == "band":
        ...
        for repeat in range(maximum_repeat_count):
            plot_parts.append(
                "{} using 1:{} with linespoints lw 1 pt 6 ps 0.45 dt 2 "
                "title 'Repeat {} median'".format(
                    gnuplot_quote(data_path), 5 + repeat, repeat))
    plot_parts.append(
        "{} using 1:2 with linespoints lw 2.5 pt 7 ps 0.6 "
    ...
    with open(path, "w", encoding="utf-8", newline="\n") as stream:
        ...
        stream.write("set xrange [{}:{}]\n".format(
            aggregated[0]["offset"], aggregated[-1]["offset"]))
        ...
        stream.write("set xlabel 'Probe offset from region base (bytes, log_2 scale)'\n")
        stream.write(
...
def write_provenance(path, input_path, input_digest, output_path,
    ...
    try:
        with open(path, "x", encoding="utf-8", newline="\n") as stream:
            ...
            stream.write("plot_type=spatial-locality-curve\n")
            ...
            stream.write("maximum_repeat_count_per_probe_offset={}\n".format(
                maximum_repeat_count))
            stream.write("unique_probe_offset_count={}\n".format(point_count))
            ...
            stream.write("cache_line_size_annotated=false\n")
...
def create_plot(arguments, metadata, trial_count, aggregated,
    ...
    with tempfile.TemporaryDirectory(
            prefix=".ece592-spatial-plot-", dir=output_parent) as temporary:
        data_path = os.path.join(temporary, "spatial.dat")
        script_path = os.path.join(temporary, "spatial.gnuplot")
        temporary_pdf = os.path.join(temporary, "spatial.pdf")
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Plot an unannotated Phase-I randomized spatial curve.")
...
def main():
    ...
    print("maximum_repeat_count_per_probe_offset={}".format(
        maximum_repeat_count))
    print("unique_probe_offset_count={}".format(len(aggregated)))
    print("cache_line_size_annotated=false")
    print("status=ok")
    return 0

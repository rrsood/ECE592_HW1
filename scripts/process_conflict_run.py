#!/usr/bin/env python3
"""Validate and summarize every point in a completed conflict sweep."""
...
def validate_manifest(plan_rows, manifest_rows):
    ...
    for plan_row, manifest_row in zip(plan_rows, manifest_rows):
        ...
        if manifest_row["status"] != "complete" or manifest_row["return_code"] != 0:
            raise ValueError(
                "conflict sweep is not complete at global index {}".format(
...
def validate_raw_point(metadata, plan_row, raw_filename, build_command,
    ...
    require_raw_integer(
        metadata, "conflict_line_count", plan_row["conflict_line_count"])
    require_raw_integer(
        metadata, "conflict_stride_bytes", plan_row["conflict_stride_bytes"])
    require_raw_integer(metadata, "random_seed", plan_row["point_seed"])

    minimum_warmup_batches = (
        plan_row["conflict_line_count"] + accesses_per_sample - 1) // accesses_per_sample
    ...
    if raw_warmup_batches < minimum_warmup_batches:
        raise ValueError(
            "raw warm-up does not cover one complete conflict cycle")

    for key in COMMON_RAW_KEYS:
        if key not in metadata:
            raise ValueError("raw file is missing common metadata {}".format(key))

    if metadata["traversal"] != "randomized-conflict-dependent-cycle":
    ...
    if metadata["fixed_low_address_bits"] != "true":
        raise ValueError("fixed_low_address_bits must be true")
...
def process_points(run_directory, plan_rows, manifest_rows, build_command):
    ...
    for plan_row, manifest_row in zip(plan_rows, manifest_rows):
        ...
        validate_raw_point(
            metadata, plan_row, raw_filename, build_command, accesses)
        ...
        row = {
            ...
            "conflict_line_count": plan_row["conflict_line_count"],
            "conflict_stride_bytes": plan_row["conflict_stride_bytes"],
    ...
    processed_rows.sort(key=lambda row: (
        row["trial"], row["conflict_stride_bytes"],
        row["conflict_line_count"], row["global_execution_index"]))
...
def write_processed(path, run_directory, plan_path, manifest_path,
    ...
    try:
        with open(path, "x", encoding="utf-8", newline="\n") as stream:
            created = True
            stream.write("ece592_conflict_processed_format_version=1\n")
            stream.write("source_run_directory={}\n".format(run_directory))
            stream.write("source_conflict_plan_sha256={}\n".format(
            ...
            stream.write("row_order=trial then conflict_stride then conflict_line_count\n")
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Validate and summarize a completed conflict-run directory.")
...
def main():
    ...
    plan_path = os.path.join(arguments.run_directory, "conflict_plan.tsv")
    ...
    try:
        ...
        plan_metadata, numeric_plan_metadata, plan_rows = (
            run_conflict_plan.read_plan(plan_path))
        del numeric_plan_metadata
        build_command = run_conflict_plan.read_build_command(build_command_path)
